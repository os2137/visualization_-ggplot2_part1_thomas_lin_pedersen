...
### Stat
stat = preplotting transformation of your data
...
```{r}
# Using geom_stat instead of geom_bar
data("mpg")
ggplot(mpg) + 
...
# guides dont behave very well as of now, it still have legacy behavior & documentation, thomas plans to improve it in the later version. 
* * *
...
# As you can with different axis labels its less effective in communicating the underlying data.
```


```{r}
# we can free one of the scales 
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
...
ggplot(mpg) + 
  ...
  facet_grid(class ~ .)
# A lot of space being not used and this makes the graphs also horrible. 
```
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_wrap(~ drv + year)
# this is the less effective way of showing the date than the facet_grid chunk above. 
```
